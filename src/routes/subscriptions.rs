...
// debug gives as much information as possible
pub struct StoreTokenError(sqlx::Error);

// display gives a brief description (user facing)
...
impl std::error::Error for StoreTokenError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        // the compiler casts &sqlx::error into &dyn error
        Some(&self.0)
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SubscribeError::PoolError(_) => {
                write!(f, "Failed to acuire a Postgres connection from the pool.")
            }
...
#[tracing::instrument(
    name = "Send a conrifmation email to a new subscriber",
